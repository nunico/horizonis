describe('Zoom UX Regression', () => {
    ...
    it('should respect max zoom limit on focused object', async () => {
        ...
        await browser.performActions([{
            ...
            actions: [
                { type: 'pointerMove', duration: 100, x: Math.round(planetPos.x), y: Math.round(planetPos.y) }
            ]
        }]);
                ...
                if (node.parentId) {
                    ...
                    if (parent) {
                        ...
                        if (childVisRadius > parentVisRadius * 0.4001) {
                            errors.push(`${node.body.name} (${childVisRadius.toFixed(2)}) is too large compared to parent (${parentVisRadius.toFixed(2)}) - ratio: ${(childVisRadius/parentVisRadius).toFixed(3)}`);
                        }
                    }
                }
            }
