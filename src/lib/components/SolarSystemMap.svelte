<script lang="ts">
  ...
  function renderSystem() {
    ...
    for (const star of systemData.stars) {
        ...
        const color = star.spectral_class.startsWith('G') ? 0xfde047 : 
                     star.spectral_class.startsWith('M') ? 0xf97316 : 0x38bdf8;
...
<div class="w-full h-full relative">
    ...
    <div class="absolute top-4 left-4 flex gap-2">
        ...
        <div class="bg-slate-800 p-1 rounded-lg border border-slate-700 flex gap-1">
            <button 
                class="px-3 py-1 rounded-md text-sm transition-colors {scaleConfig.mode === 'linear' ? 'bg-sky-600 text-white' : 'text-slate-400 hover:text-slate-200'}"
                on:click={() => setMode('linear')}
            >Linear</button>
            <button 
                class="px-3 py-1 rounded-md text-sm transition-colors {scaleConfig.mode === 'log' ? 'bg-sky-600 text-white' : 'text-slate-400 hover:text-slate-200'}"
                on:click={() => setMode('log')}
            >Log</button>
        </div>
    </div>
    {#if systemData}
    <div class="absolute bottom-4 left-4 pointer-events-none">
        ...
        <p class="text-slate-500 uppercase text-xs tracking-widest font-semibold mt-1">Solar Scale: {scaleConfig.mode}</p>
    </div>
    {/if}
</div>
