<script lang="ts">
  ...
  async function handleSave() {
    ...
    for (const system of newCluster.systems) {
        ...
        const updateBody = (bodies: any[]): boolean => {
...
{#if entity}
<div class="absolute top-4 right-4 w-80 bg-slate-900/90 backdrop-blur-md border border-slate-700 rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[calc(100vh-2rem)]">
    <div class="p-4 border-b border-slate-700 flex justify-between items-center bg-slate-800/50">
        <h2 class="font-bold text-slate-100 uppercase text-xs tracking-widest">Inspector</h2>
        <button on:click={() => selectedEntity.set(null)} class="text-slate-500 hover:text-slate-300 transition-colors">
            <X size={18} />
        </button>
    </div>
    ...
    <div class="p-4 space-y-4 overflow-y-auto">
        <div>
            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-tighter mb-1" for="name">Name</label>
            <input 
                id="name"
        ...
        <div>
            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-tighter mb-1">Spectral Class</label>
            <p class="text-slate-300 font-mono bg-slate-950/50 px-2 py-1 rounded border border-slate-800">{entity.spectral_class}</p>
        </div>
        {/if}

        {#if entity.body_type}
        <div>
            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-tighter mb-1">Type</label>
        ...
        <div>
            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-tighter mb-1">Orbit Radius</label>
        ...
        <div>
            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-tighter mb-1">Tags</label>
            <div class="flex flex-wrap gap-1 mt-1">
                {#each entity.tags as tag}
                <span class="flex items-center gap-1 bg-sky-900/30 text-sky-400 border border-sky-800/50 px-2 py-0.5 rounded text-[10px] font-medium">
                    <Tag size={10} /> {tag}
                </span>
                {/each}
            </div>
