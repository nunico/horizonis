...
- The active map style also themes the HTML/CSS UI chrome: Tailwind's `slate`/`sky` palettes are backed by CSS variables (`rgb(var(--slate-N) / <alpha-value>)`) with defaults (stock Tailwind = the realistic look) in `app.css`. A style's optional `ui` block (`StyleDefinition.ui`: font + per-shade ramp overrides) is applied to `document.documentElement` by `applyUiTheme` (`$lib/styles/ui.ts`) from the root layout, re-skinning every component that uses those classes with no per-component edits (2026-06-17).
